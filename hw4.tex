...
{\large Due: Thursday Feb 26}
\end{center}

\Aufg Prove that $A \leq_m B$ and $B$ c.e.\ imply that $A$ is c.e. Use this to show that $\operatorname{Fin}$ and $\operatorname{Cof}$ are not c.e. 



\Aufg Show that there are infinitely many sets that are c.e.\ but not computable.
