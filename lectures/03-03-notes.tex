...
\[ (\Nat, \Pow{\Nat},    \epsilon, +, *, 0, 1) \]%  ^two sorted structure ^relation between number and set variables 
...
parially ordered: $\underbar{a} \le \underbar{b}$ % should be undertilde
...
\begin{proof}
 ...
 Def. \[
       ...
       \]
\begin{align*}
 d \in A => Phi^A_d(d) \downarrow = f(d) = Phi^A_d(d) +1 >< \\ % contradiction
 d \not\in A => Phi^A_d(d) \uparrow = f(d) >< %contradiction
